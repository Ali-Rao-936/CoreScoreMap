...
class WebService {
    ...
    func specialRequest(url: String, method: Alamofire.HTTPMethod, params: Dictionary<String, Any>?, completion: @escaping (JSON?) -> Void, failed: @escaping (String) -> Void)
   {
       print(url)
       if(params != nil)
       {
    ...
    private func getHeaders(url urlString: String) -> HTTPHeaders{
       ...
       headers["language"] = Utility.getCurrentLang()
